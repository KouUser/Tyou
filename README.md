...
（如果对你有帮助，给个star就好了。 有问题反馈 qq 499793702  TEngine技术群 862987645）
...
框架提供了一套完整的 **从 PSD 设计稿到可运行 UI 代码** 的自动化流水线，中间只需人工介入“调键点/改前缀”一步（暂不支持九宫格切割等细微优化也需要手工调整一下）：
...
- **调整层**（亮度/对比度等）自动跳过
- 建议美术在 PSD 中就按 `m_btn`、`m_text` 等前缀规范命名图层，可省去导入后手动改名的步骤
